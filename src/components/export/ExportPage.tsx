...
export default function ExportPage() {
  const { goals, entries, dailyLogs, reviews, milestones, settings } = useAppStore();
  ...
  async function exportPDF(type: 'full' | string) {
    ...
    try {
      ...
      for (const goal of goalsToExport) {
        ...
        doc.setFontSize(7);
        doc.setTextColor(...muted);
        doc.text(goal.priority.toUpperCase(), 15, y + 5);
        y += 12;
  ...
  return (
    <div className="min-h-screen bg-black page-enter pb-8">
      <div className="sticky top-0 bg-black border-b border-border px-4 py-3 flex items-center gap-3">
        ...
        <div className="font-display text-accent text-sm tracking-widest">EXPORT</div>
      </div>
